...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Estimations des PIB", tabName = "statistiques", icon = icon("bar-chart"),
  ...
  dashboardBody(
    tabItems(
              ...
              fluidRow(
                ...
                box(title = "Indicateurs clés", width = 12, status = "info", solidHeader = TRUE,
                    fluidRow(
                      ...
                      valueBoxOutput("cadran_region_eclairee", width = 4)
                    )
                ),
                box(title = "Explorer les Graphiques", width = 12, solidHeader = TRUE,
                    selectInput("choix_graphique", "Choisissez un graphique :", 
                                choices = c("Distribution des intensités lumineuses par région" = "distribution",
                                            "Top 10 des départements les plus éclairés" = "top_departement",
                                            "Top 10 des communes les plus éclairées" = "top_commune")),
      ...
      tabItem(tabName = "pib_estime",
              fluidRow(
                box(title = "Tableau : PIB Estimé par Région", width = 12, solidHeader = TRUE,
                    DT::dataTableOutput("table_pib")),
                box(title = "Graphiques : Répartition du PIB Estimé", width = 12, solidHeader = TRUE,
      ...
      tabItem(tabName = "chomage_estime",
              fluidRow(
                box(title = "Table des Taux de Chômage", width = 12, solidHeader = TRUE,
                    DT::dataTableOutput("table_chomage")),
                box(title = "Répartition du Taux de Chômage", width = 12, solidHeader = TRUE,
      ...
      tabItem(tabName = "luminosite",
              fluidRow(
                box(title = "Carte des intensités lumineuses par région (Satellite)", width = 6, solidHeader = TRUE, status = "primary",
                    imageOutput("image_satellite"),
                    p("Source : Visible Infrared Imaging Radiometer Suit, AIDDATA", style = "font-size: 12px; color: gray;")
                ),
                box(title = "Carte des intensités lumineuses par région (SIG)", width = 6, solidHeader = TRUE, status = "success",
      ...
      tabItem(tabName = "equipe",
              fluidRow(
                ...
                box(title = "Concepteur 1", width = 4, solidHeader = TRUE,
                    img(src = "concepteur1.jpg", height = "150px", width = "150px"),
                    p("Biographie de concepteur 1... Il/Elle est spécialisé(e) dans..."))
                ,
                # Biographie du concepteur 2
                box(title = "Concepteur 2", width = 4, solidHeader = TRUE,
                    img(src = "concepteur2.jpg", height = "150px", width = "150px"),
                    p("Biographie de concepteur 2... Il/Elle est spécialisé(e) dans..."))
                ,
                # Biographie du concepteur 3
                box(title = "Concepteur 3", width = 4, solidHeader = TRUE,
                    img(src = "concepteur3.jpg", height = "150px", width = "150px"),
                    p("Biographie de concepteur 3... Il/Elle est spécialisé(e) dans..."))
              )
      )
    )
  )
...
server <- function(input, output) {
  ...
  output$graphique_accueil <- renderPlotly({
    if (input$choix_graphique == "distribution") {
      ...
      plot_ly(region_data, x = ~region, y = ~intensite_moyenne, type = 'bar') %>%
        layout(title = "Distribution des Intensités Lumineuses par Région",
      ...
      plot_ly(top_depts, x = ~departement, y = ~intensite_totale, type = 'bar') %>%
        layout(title = "Top 5 Départements les Plus Éclairés",
      ...
      plot_ly(top_communes, x = ~commune, y = ~intensite_totale, type = 'bar') %>%
        layout(title = "Top 5 Communes les Plus Éclairées",
  ...
  output$image_satellite <- renderImage({
    list(src = "/www/image_satellite.jpg", contentType = "image/jpg", width = "100%")
  }, deleteFile = FALSE)
  
  output$image_SIG <- renderImage({
    list(src = "/www/image_SIG.jpg", contentType = "image/jpg", width = "100%")
  }, deleteFile = FALSE)
